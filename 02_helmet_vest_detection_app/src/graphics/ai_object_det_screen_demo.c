...
#include "ign_logo.c"
#include "helmet_vest_detection_title.c"
...
static st_ai_detection_point_t g_ai_detection_copy[20];
...
//    static uint64_t t1 = 0;
    ...
    if(in_transition())
    {
		...
		d2_setblitsrc(d2_handle, helmet_vest_detection_title.pixel_data, helmet_vest_detection_title.width, helmet_vest_detection_title.width, helmet_vest_detection_title.height, EP_SCREEN_MODE);

		d2_blitcopy(d2_handle,
				helmet_vest_detection_title.width, helmet_vest_detection_title.height,  // Source width/height
				(d2_blitpos) 0, 0, // Source position
...
//        print_bg_font_18(d2_handle, 180, 260,  (char*)"Neural network - FastestDet");
    ...
    {
	print_bg_font_18(d2_handle, 350, 680,  (char*)"persons without ");
	print_bg_font_18(d2_handle, 320, 720,  (char*)" safety");
...
//    	t1 = get_timestamp();
		...
		{
			uint8_t no_safety = 0;
			uint8_t helmet_count = 0;
			#define DET_MODEL_IMG_SIZE_X 224
			#define DET_MODEL_IMG_SIZE_Y 224
			bool overlap[20] = {false};
			bool vest_nearby[20] = {false};
			const int vest_threshold = 60;
			memcpy(g_ai_detection_copy, g_ai_detection, sizeof(g_ai_detection));
			...
			for(int i=0; i<20; i++)
			{
				...
				d2_point x = (g_ai_detection[i].m_x*640)/224;
				d2_point y = (g_ai_detection[i].m_y*480)/224;
				d2_point w = (g_ai_detection[i].m_w*640)/224;
				d2_point h = (g_ai_detection[i].m_h*480)/224;
				
				if(g_ai_detection[i].m_c==2)
				{
					for(int j=0; j<20; j++)
					{
						if(i==j || g_ai_detection_copy[j].m_c == 2)
							continue;
						d2_point x2 = (g_ai_detection_copy[j].m_x*640)/224;
						d2_point y2 = (g_ai_detection_copy[j].m_y*480)/224;
						d2_point w2 = (g_ai_detection_copy[j].m_w*640)/224;
						d2_point h2 = (g_ai_detection_copy[j].m_h*480)/224;

						if(g_ai_detection_copy[j].m_c == 1)
						{
							d2_point head_x = x + w / 2;
							d2_point head_y = y + h;
							d2_point vest_x = x2 + w2 / 2;
							d2_point vest_y = y2;
							double vest_distance = sqrt(pow(head_x - vest_x, 2) + pow(head_y - vest_y, 2));

							if(vest_distance < vest_threshold)
							{
								vest_nearby[i] = true;
							}
						}

						if(g_ai_detection_copy[j].m_c == 0)
						{
				...
				//0 helmet 1 vest 2 head
				if(g_ai_detection[i].m_c == 0)
					d2_setcolor(d2_handle, 0, 0x800080);
				else if(g_ai_detection[i].m_c == 1)
					d2_setcolor(d2_handle, 0, 0x0000FF);
				else if(g_ai_detection[i].m_c == 2 )
				{
					if(overlap[i] && vest_nearby[i])
						d2_setcolor(d2_handle, 0, 0x00FF00); // green if helmet is on and vest is nearby
					else
					{
						d2_setcolor(d2_handle, 0, 0xFF0000); // red otherwise
						no_safety++;
					}
				}
