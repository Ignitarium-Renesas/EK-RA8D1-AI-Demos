...
namespace image {
    ...
    void CalculateNMS(std::vector<Detection>& detections, int classes, float iouThreshold)
    {
        int idxClass{0};
        auto CompareProbs = [idxClass](const Detection& prob1, const Detection& prob2) {
            return prob1.prob[idxClass] > prob2.prob[idxClass];
        };

        for (idxClass = 0; idxClass < classes; ++idxClass) {
            std::sort(detections.begin(), detections.end(), CompareProbs); // Use std::sort
    ...
    void ConvertImgToInt8(void* data, const size_t kMaxImageSize, float scale, int zero_point)
    {
...
//                    (int32_t) (tmp_req_data[i]/(scale*255)) + zero_point);
            		(int32_t) (tmp_req_data[i]) + zero_point);
//            debug("after %d\n", tmp_req_data[i]);
        }
    }
