...
bool DetectorPostProcess::DoPostProcess()
{
    ...
    std::vector<image::Detection> detections;
    ...
    for (auto& it: detections) {
        ...
        //for (int j = 0; j < this->m_net.numClasses; ++j) {
            if (it.prob[0] > 0) {
                ...
                tmpResult.m_normalisedVal = it.prob[0];
                tmpResult.m_x0 = boxX;
                tmpResult.m_y0 = boxY;
                tmpResult.m_w = boxWidth;
                tmpResult.m_h = boxHeight;
                tmpResult.m_class_idx = it.class_idx;

                this->m_results.push_back(tmpResult);
                //e_printf("m_class_idx -> %d, score -> %f\n", tmpResult.m_class_idx, tmpResult.m_normalisedVal);
            }
        //}
    }
...
// void DetectorPostProcess::InsertTopNDetections(std::vector<image::Detection>& detections, image::Detection& det)
// {
//     std::vector<image::Detection>::iterator it;
//     std::vector<image::Detection>::iterator last_it;
        ...
        std::vector<image::Detection>& detections)
{
    ...
    for (size_t i = 0; i < net.branches.size(); ++i) {
        ...
        for (int idx = 0; idx < (height*width*channel); idx = idx+channel){
			...
			{
				float cls_score = (static_cast<float>(net.branches[i].modelOutput[idx+5+j])
						- net.branches[i].zeroPoint)
						* net.branches[i].scale;
