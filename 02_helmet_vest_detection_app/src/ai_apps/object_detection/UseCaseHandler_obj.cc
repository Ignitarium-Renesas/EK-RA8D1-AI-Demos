...
extern "C" {
#include "timer.h"
void update_detection_result(signed short index, signed short  x, signed short  y, signed short  w, signed short  h, unsigned short c);
}
...
//    		e_printf("results[i].m_class_idx -> %d\n", results[i].m_class_idx);
            update_detection_result(i, results[i].m_x0, results[i].m_y0, results[i].m_w, results[i].m_h, results[i].m_class_idx);
        }
