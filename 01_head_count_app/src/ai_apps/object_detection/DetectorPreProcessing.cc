    ...
    bool DetectorPreProcess::DoPreProcess(const void* data, size_t inputSize) {
        ...
        if (this->m_convertToInt8) {
            image::ConvertImgToInt8(this->m_inputTensor->data.data, this->m_inputTensor->bytes,
            		(static_cast<TfLiteAffineQuantization*>(this->m_inputTensor->quantization.params))->scale->data[0],
					(static_cast<TfLiteAffineQuantization*>(this->m_inputTensor->quantization.params))->zero_point->data[0]);
        }
