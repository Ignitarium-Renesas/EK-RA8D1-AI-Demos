...
namespace object_detection {
    ...
    public:
        ...
        DetectionResult(double normalisedVal,int32_t x0,int32_t y0, int32_t w,int32_t h,
        				int32_t box_id_num, int32_t box_id_count, int32_t inactive_frames) :
                m_normalisedVal(normalisedVal),
                m_x0(x0),
                m_y0(y0),
                m_w(w),
                m_h(h),
				m_box_id_num(box_id_num),
				m_box_id_count(box_id_count),
				m_inactive_frames(inactive_frames),
				m_is_matched(false)
            {
            }
        ...
        int32_t     m_w{0};
        int32_t     m_h{0};
        int32_t     m_box_id_num{0};
        int32_t     m_box_id_count{0};
        int32_t     m_inactive_frames{0};
        bool        m_is_matched{false};
    };
