...
void button_irq10_callback(external_irq_callback_args_t *p_args)
{
    ...
    if ((uxBits & (STATUS_UPDATE_INTENSE_INFO)) != (STATUS_UPDATE_INTENSE_INFO))
    {
        ...
        xResult = xEventGroupSetBitsFromISR(g_update_console_event, STATUS_UPDATE_INTENSE_INFO, &xHigherPriorityTaskWoken);
        xResult = xEventGroupSetBitsFromISR(g_update_console_event, STATUS_HMI_NEXT_DEMO, &xHigherPriorityTaskWoken);
...
void button_irq9_callback(external_irq_callback_args_t *p_args)
{
    ...
    if ((uxBits & (STATUS_UPDATE_FREQ_INFO)) != (STATUS_UPDATE_FREQ_INFO))
    {
        ...
        xResult = xEventGroupSetBitsFromISR(g_update_console_event, STATUS_HMI_PREV_DEMO, &xHigherPriorityTaskWoken);
